           ...
           <div className="mail-list-container col-md-3 pt-4 pb-4 border-right bg-white">
                  <div className="border-bottom pb-4 mb-3 px-3">
                      <div className="form-group">
                          <input className="form-control w-100" type="search" placeholder="Search notes" id="Mail-rearch"
                          value={search}
                          name="search"
              ...
              {notes.slice(0, 5).filter((val)=>{
                if(search === ''){
                  return val;
                }else if (val.work_title.toLowerCase().includes(search.toLowerCase())){
                  return val;
                }
              }).slice(0, 5).map((note)=>(
